...
void FILTROS::fftImgOrigen()
{
	...
	if (!already_transformed) {
		if (input_already_set) {
			...
			for (unsigned int i = 0; i < my_filters_imgs_count; i++) {
				*(Img_fft + i) = (fftwl_complex*)fftw_malloc((my_img_base->at(i)).getHeight() *
					(my_img_base->at(i)).getWidth() * sizeof(fftwl_complex));

				*(Img_fft_HPF + i) = (fftwl_complex*)fftw_malloc((my_img_base->at(i)).getHeight() *
					(my_img_base->at(i)).getWidth() * sizeof(fftwl_complex));
			}
			...
			for (unsigned int i = 0; i < my_filters_imgs_count; i++) {
				...
				for (unsigned int y = 0; y < my_img_height; y++) {
					for (unsigned int x = 0; x < my_img_width; x++) {
						*(*(Img_org + x + y * my_img_width)) = 1.0 - (my_img_base->at(i)).getPix(y, x);
						*(*(Img_org + x + y * my_img_width)+1) = 0.0;
					}
				}

				fftwl_plan p_r2c = fftwl_plan_dft_2d(my_img_height, my_img_width, Img_org, *(Img_fft + i), FFTW_FORWARD,  FFTW_ESTIMATE);
...
void FILTROS::respGabor() {
	...
	for (long double theta = 0.0; theta < 180.0; theta += theta_increment, k++) {
		...
		for (unsigned int i = 0; i < my_filters_imgs_count; i++) {
			...
			p_c2r = fftwl_plan_dft_2d((my_img_base->at(i)).getHeight(), (my_img_base->at(i)).getWidth(), Img_filter, Img_resp, FFTW_FORWARD, FFTW_ESTIMATE);
			fftwl_execute(p_c2r);
